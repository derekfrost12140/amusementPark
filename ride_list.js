document.addEventListener('DOMContentLoaded', async () => {
  ...
  function createRideElement(ride) {
    ...
    accessibilityText.push(`<span class='filter-tag' data-filter='touch' data-number='${ride.touch}'>Touch Level: ${ride.touch}</span>`);
    accessibilityText.push(`<span class='filter-tag' data-filter='taste' data-number='${ride.taste}'>Taste Level: ${ride.taste}</span>`);
    accessibilityText.push(`<span class='filter-tag' data-filter='sound' data-number='${ride.sound}'>Sound Level: ${ride.sound}</span>`);
    accessibilityText.push(`<span class='filter-tag' data-filter='smell' data-number='${ride.smell}'>Smell Level: ${ride.smell}</span>`);
    accessibilityText.push(`<span class='filter-tag' data-filter='sight' data-number='${ride.sight}'>Sight Level: ${ride.sight}</span>`);
    accessibilityData.innerHTML = accessibilityText.join('');
  ...
  await db.collection("rides").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
      const rideData = doc.data();
      createRideElement(rideData);
    });
  }).catch((error) => {
  ...
  /*********************************
   * 5. Create & Handle Filter Modal
   *********************************/
  const filterBtn = document.getElementById('filterBtn');
  const preferencesModal = document.createElement('div'); // dynamic modal
  ...
  // Close filter modal -> revert changes
  ...
  document.addEventListener('click', (event) => {
    if (event.target && event.target.classList.contains('filter-tag')) {
      ...
      // If it's a number input (like tasteNum, etc.)
